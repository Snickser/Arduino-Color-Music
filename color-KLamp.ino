...
byte modeL = 1;
float cLim = 200;    // color for 1/random() 3,6,12,24,48,96,192,384,768
unsigned int rainbow = 12;
float colorST = 0.5;
...
#define LED_BUILTIN 17
  ...
  EEPROM.get(4, colorST);
  if (isnan(colorST)) colorST = 0.5;
  ...
  /*
    while(1){
      for(byte i=0;i<PixelCount;i++){
        RgbColor color = HsbColor(1-i/192., 1, Br);
    //      RgbColor color = RgbColor(255-i,i,0);
         strip.SetPixelColor(i, color);
         Serial.println(1-i/192.,6);
      }
        strip.Show();
        delay(10000);
    }
...
void loop() {
  ...
  // random Mode
  if ( (tNow - timeL) / 1000 > modeR && Mode != 0 && Mode != 8 && Mode != 9) {
    timeL = tNow;
    modeR = random(29, 300);
    modeL = Mode;
    while ( Mode == modeL ) Mode = random(1, 8);
    rainbow = random(6, cLim);
    nRB = 1;
...
void setcolor(float x) {
  RgbColor color = HsbColor(x, 1, Br);
  strip.ClearTo(color); // вся лента
  strip.Show();
}
